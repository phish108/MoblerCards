...
LMSView.prototype.createLMSItem = function (lmsData) {
    ...
    if (selectedLMS === lmsData.logoLabel) {
        lmstmpl.lmselement.addClass("gradientSelected");
    }
    else {
        lmstmpl.lmselement.addClass("gradient2");
        lmstmpl.lmselement.addClass("textShadow");
    }
    
    if (lmsModel.isRegistrable(sn)) {
        lmstmpl.lmswait.addClass("hidden");
        lmstmpl.lmsdash.addClass("select");
    }
    else {
        lmstmpl.lmsimg.addClass("hidden");
        lmstmpl.lmswait.addClass("icon-cross");
        lmstmpl.lmsdash.addClass("dashGrey");
