...
function queryDatabase(cbResult) {
    console.log("enter queryDatabase " + this.modelName);
    var self = this;
    self.superModel.db.transaction(function (transaction) {
        transaction.executeSql(self.query, self.values, cbResult, function (tx, e) {
            console.log("DB Error cause: " + self.modelName);
            self.superModel.dbErrorFunction(tx, e);
        });
    });
}
    ...
    self.superModel.db.transaction(function (transaction) {
        transaction.executeSql("SELECT * FROM statistics WHERE course_id = ? AND question_id = ?", [self.courseId, self.achievementName],
            function cbSuccess(t, r) {
                if (r.rows.length > 0) {
                    console.log("found " + self.achievementName + " in the local database");
                    self.achievementValue = 100;
                    self.superModel.allDone();
                } else {
                    console.log("no " + self.achievementName + " in the local database");
...
function getCorrectGaps(gapIndex) {
    ...
    for (k = 0; k < jQuery(items).size(); k++) {
        ...
        console.log("item of correct gaps array is " + correctGaps[k]);
    }
    console.log("correct gaps array is " + correctGaps);
    return correctGaps;
}
